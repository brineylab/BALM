...
class TopKRouter(BaseRouter):
    ...
    def forward(self, x: torch.Tensor, k: int, expert_capacity: int, sort_topk: bool = True):
        """
        Parameters:
        -----------
        x : torch.Tensor
...
v
        ...
        topk_probs : torch.Tensor
            Selected token probabilities for each expert `(num_tokens, k)`.
        topk_mask : torch.Tensor
            Token indices assigned to each expert `(num_tokens, k, num_experts)`.
        """
        ...
        # permute & flatten ==> (k * num_tokens)
        # groups by k: [top 1 of all tokens] + [top 2 of all tokens] etc
        flat_unnorm_probs = unnorm_topk_probs.permute(1, 0).reshape(-1)
        flat_expert_ids = topk_expert_ids.permute(1, 0).reshape(-1)

        # sort by prob
        if sort_topk:
            _, sort_idx = flat_unnorm_probs.sort(descending=True)
            flat_expert_ids = flat_expert_ids[sort_idx]

        # expert mask
        topk_mask = F.one_hot(flat_expert_ids, num_classes=num_experts)
        
        # mask tokens beyond capacity
        if expert_capacity != -1:
            sum_mask = topk_mask.cumsum(dim=0)
            mask = (sum_mask <= expert_capacity)
            topk_mask = topk_mask * mask

        # unsort
        if sort_topk:
            unsort_idx = sort_idx.argsort()
            topk_mask = topk_mask[unsort_idx] 

        # reshape ==> (num_tokens, k, num_experts)
        topk_mask = topk_mask.view(k, num_tokens, num_experts).permute(1, 0, 2)

        return logits, probs, topk_probs, topk_mask
...
class TopPRouter(BaseRouter):
    ...
    def forward(self, x: torch.Tensor, expert_capacity: int):
        ...
        topp_probs = torch.where(mask, 0.0, sorted_probs)

        # TODO: can't one hot the sorted_indices array...
        # topp_mask = F.one_hot(sorted_indices, num_classes=num_experts)
        
        # mask tokens beyond capacity
        if expert_capacity != -1:
            sum_mask = topp_mask.cumsum(dim=0)
            mask = (sum_mask <= expert_capacity)
            topp_mask = topp_mask * mask

        raise Exception()

        return logits, probs, topp_probs, topk_mask
